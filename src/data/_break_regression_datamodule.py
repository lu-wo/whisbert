...
from src.data.components.feature_extractors import WordBreakFeatureExtractor
...
class WordBreakRegressionDataModule(LightningDataModule):
    """
    ...
    def __init__(
        ...
        """
        Default is to predict breaks before
        """
        super().__init__()
    ...
    def prepare_dataset(self, file_name):
        extractor = WordBreakFeatureExtractor(
        ...
        if self.hparams.breaks_after:
            breaks = extractor.get_all_breaks_after()
        else:
            breaks = extractor.get_all_breaks_before()
