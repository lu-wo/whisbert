...
class EnergyRegressionDataModule(LightningDataModule):
    """
    ...
    def prepare_dataset(self, file_name):
        ...
        data_id = f"energy_{self.hparams.energy_mode}"
        ...
        if os.path.exists(data_cache_path) and data_id in os.listdir(data_cache_path):
            ...
            extractor = ProsodyFeatureExtractor(
                ...
                extract_energy=True,
                energy_mode=self.hparams.energy_mode,
            )
            texts = extractor.get_all_text()
            energy = extractor.get_all_energy()

            # Save the data to cache
            data = {"texts": texts, "energy": energy}
