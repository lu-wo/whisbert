...
def step(model, loss_fn, data, target, optimizer=None):
    data, target = data.to(DEVICE), target.to(DEVICE)
    outputs = model(data)
    mu, sigma = outputs[:, 0], torch.exp(outputs[:, 1])
    # check for nans in mu and sigma
    if torch.isnan(mu).any() or torch.isnan(sigma).any():
        print("Nans in mu or sigma")
        print("mu", mu)
        print("sigma", sigma)

    loss = gnll_loss(outputs, target)
    ...
    mae = F.l1_loss(mu, target)
    r2 = r2_score(
        target.cpu().detach().numpy(), mu.cpu().detach().numpy()
    )  # Added R2 calculation

    return loss.item(), mae.item(), r2  # return R2 score
...
def train_and_test(args, model, word_model, loss_fn, learning_rate, l2_reg):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--NUM_RUNS", default=15, type=int)
