...
def step(model, loss_fn, data, target, optimizer=None, verbose=False):
    data, target = data.to(DEVICE), target.to(DEVICE)
    output = model(data)

    mean, std = torch.nn.functional.softplus(
        output[:, 0]
    ), torch.nn.functional.softplus(output[:, 1])
    dist = Gamma(mean, std)

    target = target + 1e-4
    loss = -dist.log_prob(target).mean()  # negative log likelihood
    pred = dist.mean
