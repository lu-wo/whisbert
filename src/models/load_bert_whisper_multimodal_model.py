...
class BertWhisperTrainingModule(LightningModule):
    def __init__(
        ...
        if use_pretrained:
            ...
            self.text_encoder = BertForMaskedLM.from_pretrained(huggingface_model)
            self.tokenizer = AutoTokenizer.from_pretrained(huggingface_model)

            # TODO: implement Whisper is 512 in deim, BERT 768
            ...
            # self.audio_encoder.load_weights(whisper_pre.encoder)
        else:
    ...
    def _unimodal_step(self, audio_inputs, text_inputs, batch_idx, verbose=False):
        print(f"\nUnimodal step {batch_idx}")
        loss = 0.0
        audio_loss, text_loss = None, None

        if self.hparams.mam_weight:
    ...
    def audio_l2_loss(self, preds, labels, mask):
        """
    ...
    def _multimodal_step(self, audio_inputs, text_inputs, batch_idx, verbose=False):
        print(f"\nMultimodal step for batch {batch_idx}")
    ...
    def training_step(self, batch, batch_idx):
        ...
        if random.random() > self.hparams.unimodal_prob:
            ...
            loss, audio_loss, text_loss = self._unimodal_step(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if random.random() > self.hparams.unimodal_prob:
            ...
            if text_loss is not None:
                ...
                self.log(
                    "val/loss", text_loss, on_step=True, on_epoch=True, prog_bar=True
                )
