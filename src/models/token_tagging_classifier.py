...
class TokenTaggingClassifier(LightningModule):
    ...
    def __init__(
        ...
        output_activation: nn.Module = torch.nn.Softmax(),
        save_path: str = None,
    ):
        super().__init__()
        ...
        if use_mlp:
            ...
            self.regressor = nn.Linear(self.model.config.hidden_size, nb_classes)
        ...
        self.train_loss = partial(
            masked_loss, loss_fn=torch.nn.CrossEntropyLoss(reduction="None")
        )
        self.val_loss = partial(
            masked_loss, loss_fn=torch.nn.CrossEntropyLoss(reduction="None")
        )
        self.test_loss = partial(
            masked_loss, loss_fn=torch.nn.CrossEntropyLoss(reduction="None")
        )
    ...
    def step(self, batch: Dict[str, torch.tensor]):
        logits = self(batch)  # forwards pass, shape: (bs, seqlen, nb_classes)
    ...
    def training_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        ...
        self.train_acc(preds, batch["tokenized_labels"], batch["loss_mask"])
        self.log(
    ...
    def validation_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        ...
        self.val_acc(preds, batch["tokenized_labels"], batch["loss_mask"])
    ...
    def on_validation_epoch_end(self):
        ...
        self.log("val/acc_best", self.val_r2_best.compute(), prog_bar=True)
    ...
    def test_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        ...
        self.test_acc(preds, batch["tokenized_labels"], batch["loss_mask"])
