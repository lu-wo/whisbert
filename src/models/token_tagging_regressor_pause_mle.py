...
class TokenTaggingRegressorPauseMLE(LightningModule):
    """
    ...
    def step(self, batch: Dict[str, torch.tensor], verbose: bool = False):
        ...
        if verbose:
            print(f"masked neg log_likelihood loss: {masked_nll_mean}")
            print(f"masked mae: {masked_mae}")

        return masked_nll_mean, pred, var

    def training_step(
        self, batch: Dict[str, torch.tensor], batch_idx: int, verbose: bool = False
    ):
    ...
    def validation_step(
        self, batch: Dict[str, torch.tensor], batch_idx: int, verbose=False
    ):
        # loss is batch loss, val_loss tracks it over the epoch
        loss, preds, var = self.step(batch)
        if verbose:
            print(f"Labels: {batch['tokenized_labels']}")
            print(f"Preds: {preds}")
            print(f"Mask: {batch['loss_mask']}")
            print(f"Validation cond. ent: {loss}")
