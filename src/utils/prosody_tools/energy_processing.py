...
def extract_energy(
    waveform, fs=16000, min_freq=200, max_freq=3000, method="rms", target_rate=200
):
    ...
    lp_waveform = filter.butter_bandpass_filter(
        waveform, min_freq, max_freq, fs, order=5
    )
    ...
    if method == "hilbert":
        ...
        energy = np.sqrt(lp_waveform**2)  # this is not correct, I fixed it below
        # energy = np.sqrt(np.mean(lp_waveform**2, axis=-1, keepdims=True))
    logger.debug(
        "fs = %d, target_rate = %d, fs/target_rate = %f"
        % (fs, target_rate, fs / target_rate)
    )
    energy = misc.resample(energy, fs, target_rate)
    # energy = scipy.signal.resample_poly(energy, 1., int(round(fs/target_rate)))
    logger.debug(
        "len(energy) = %d, len(energy)/target_rate = %f"
        % (len(energy), len(energy) / target_rate)
    )
    return energy
