...
try:
    ...
    USE_REAPER = False
    logger.debug(
        "Pyreaper is not available so falling back into the default pitch tracker"
    )
...
def _remove_outliers(lf0, trace=False):
    ...
    boundary_cut = smooth_and_interp.interpolate_zeros(
        _cut_boundary_vals(fixed, 3), "linear"
    )
    ...
    max_win_len = 100
    min_win_len = 10  # 20
    max_threshold = 3.0  # threshold with broad window
...
def _interpolate(f0, method="true_envelope", verbose=False):
    if verbose:
        print(f"Interpolate: f0 shape {f0.shape}")
...
def extract_f0(
    ...
    # if waveform.size == 0:
    #     print("Warning: Empty waveform encountered. Returning empty F0 array.")
    #     return np.array([])

    # # Add print statements to show the waveform shape and the configuration used
    # print(f"waveform shape: {waveform.shape}")
    # print(f"configuration: {configuration}")

    if (f0_min == 0) or (f0_max == 0):
        # print(f"Extracting f0 without limits")
        if USE_REAPER and (configuration == "REAPER"):
            ...
            (f0, _) = pitch_tracker.inst_freq_pitch(
                waveform, fs, f0_min, f0_max, harmonics, voicing, False, 200
            )
    ...
    if USE_REAPER and (configuration == "REAPER"):
        if verbose:
            print(f"use reaper to extract f0")
        _, _, _, f0, _ = pyreaper.reaper(waveform, fs, f0_min, f0_max)
    else:
        if verbose:
            print(f"use pitch tracker to extract f0")
        (f0, _) = pitch_tracker.inst_freq_pitch(
            waveform, fs, f0_min, f0_max, harmonics, voicing, False, 200
        )
