...
def _get_f0(spec, energy, min_hz, max_hz, thresh, sil_thresh):
    ...
    if spec[cand] > thresh and energy > sil_thresh:
        if cand > 2 * min_hz and spec[int(round(cand / 2.0))] > spec[cand] * 0.5:
            return int(round(cand / 2.0))
        else:
            return cand
    return 0
...
def _track_pitch(
    ...
    for i in range(0, pic_smooth.shape[0]):
        pitch[i] = _get_f0(
            pic_smooth[i], log_energy[i], min_hz, max_hz, thresh, energy_thresh
        )
    ...
    for iter in range(0, n_iters):
        if verbose:
            print(f"track pitch its {iter}: {pitch.shape}")

        smoothed = f0_processing.process(pitch)
        smoothed = smooth_and_interp.smooth(smoothed, int(200.0 / (iter + 1.0)))

        # gradually thightening gaussian window centered on current estimate to softly constrain next iteration
        win_len = 800

        g_window = gaussian(win_len, int(np.mean(smoothed) * (1.0 / (iter + 1.0) ** 2)))
    ...
    if verbose:
        print(f"track pitch: {pitch.shape}")
...
def inst_freq_pitch_from_wav(
    ...
    return inst_freq_pitch(
        wav_form, fs, min_hz, max_hz, acorr_weight, voicing_thresh, DEBUG, target_rate
    )
...
def inst_freq_pitch(
    ...
    periods = [5]
    for p in periods:
        (wavelet_matrix, scales, cwt_freqs) = cwt_utils.cwt_analysis(
            tmp_wav_form,
            mother_name="morlet",
            first_scale=s0,
            num_scales=J,
        ...
        freqs = np.abs((np.gradient(phase, dt)[1]) / (2.0 * np.pi))
    ...
    for i in range(0, pic.shape[0]):  # frame
        ...
        pic[i, : int(length / 2.0)] *= acorr1[int(len(acorr1) / 2.0) :]
    ...
    if DEBUG:
        pylab.imshow(
            pic[:, 0:].T, interpolation="nearest", origin="lower", aspect="auto"
        )
